          ...
          >
            ...
            <div className="p-8 border-b border-white/5 flex justify-between items-center">
              <div className="flex flex-col gap-1">
                <h2 className="text-heading text-2xl text-white">[ WAREHOUSE_MANIFEST ]</h2>
                <p className="text-technical text-[9px] text-gray-500 uppercase tracking-widest">// ARCHIVE_TOTAL: {cart.length} PIECES</p>
              </div>
            ...
            {cart.length > 0 && (
                <div className="px-8 py-6 border-b border-white/5 bg-white/2">
                    <div className="flex justify-between items-center mb-3">
                        <p className="text-technical text-[9px] font-black tracking-widest uppercase text-white/60">
                            {remainingForFreeShipping > 0 
                                ? `// DEFICIT: ${formatPrice(remainingForFreeShipping, currency)} // TO_FREE_SHIP`
                                : '[ FREE_EXPRESS_SHIPPING_UNLOCKED ]'}
                        </p>
                        <span className="text-technical text-[9px] font-black text-white">{Math.round(shippingProgress)}%</span>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-8 scrollbar-hide" data-lenis-prevent>
              {cart.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center gap-6 text-gray-600">
                  <p className="text-technical text-[10px] tracking-[0.3em] uppercase">[ MANIFEST_EMPTY ]</p>
                  <button onClick={() => setCartOpen(false)} className="btn-secondary text-[10px] !border-white/20 !text-white hover:!bg-white hover:!text-black">// RETURN_TO_COLLECTION</button>
                </div>
              ) : (
                <>
                    <div className="flex flex-col gap-8">
                        ...
                        <div key={item.product.id} className="flex gap-6 group">
                            <div className="w-24 aspect-[4/5] bg-white/5 overflow-hidden border border-white/10 shrink-0">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <h3 className="text-technical text-[11px] font-[700] leading-tight group-hover:opacity-60 transition-opacity uppercase tracking-widest text-white">[ {item.product.name} ]</h3>
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="p-2 hover:bg-white/5 transition-colors border-r border-white/10"><Minus size={12} /></button>
                                <span className="text-technical text-[10px] px-4 font-[800] text-white">{item.quantity}</span>
                                <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="p-2 hover:bg-white/5 transition-colors border-l border-white/10"><Plus size={12} /></button>
                                </div>
                                <span className="text-technical text-[11px] font-[800] tracking-wider text-white">{formatPrice(item.product.price * item.quantity, currency)}</span>
                            </div>
                            </div>
                        </div>
                        ))}
                    ...
                    <div className="pt-12 border-t border-white/5">
                        <p className="text-technical text-[9px] font-[900] mb-8 tracking-[0.3em] uppercase text-white/40">// COMPLETE_UNIFORM</p>
            ...
            {/* Footer */}
            {cart.length > 0 && (
              <div className="p-8 border-t border-white/5 bg-[var(--color-bg-primary)] flex flex-col gap-6">
                <div className="flex flex-col gap-4">
                  <div className="flex justify-between items-center text-technical text-[10px]">
                    ...
                    <span className="font-[900] text-xl text-white">{formatPrice(cartTotal, currency)}</span>
                  </div>
                  <div className="flex justify-between items-center text-technical text-[9px] text-white">
                ...
                <button 
                  onClick={checkout}
                  className="btn-primary w-full py-5 flex items-center justify-center gap-3 !bg-white !text-black border-white"
                >
